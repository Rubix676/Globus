...
const courseLanguageSwitcher = document.getElementById("languageSwitcher");
...
const courseTranslations = {
  uz: {
    ...
    "modal.scheduleTitle": "Dars vaqtlari"
  },
  ru: {
    ...
    "modal.scheduleTitle": "Время занятий"
  },
  en: {
    ...
    "modal.scheduleTitle": "Class times"
  }
};
...
const defaultSchedule = ["14:00 - 15:30"];
...
const teacherSchedules = {
  ...
  teacher1: defaultSchedule,
  teacher3: defaultSchedule,
  teacher4: defaultSchedule,
  teacher5: defaultSchedule,
  teacher6: defaultSchedule,
  teacher7: defaultSchedule,
  teacher8: defaultSchedule
};
...
function getCurrentCourseLanguage() {
  return localStorage.getItem("siteLanguage") || courseLanguageSwitcher?.value || "uz";
}

function getCourseTranslations(language = getCurrentCourseLanguage()) {
  return courseTranslations[language] || courseTranslations.uz;
}

function translateCourseElements(language = getCurrentCourseLanguage()) {
  const selectedTranslations = getCourseTranslations(language);
...
function getTeacherLabel(teacher, selectedTranslations) {
  return teacher.label || selectedTranslations[teacher.translationKey] || teacher.value;
}
...
function setCourseTeacherOptions(courseName, selectedTranslations) {
  ...
  teachers.forEach((teacher) => {
    ...
    option.textContent = getTeacherLabel(teacher, selectedTranslations);
...
function ensureScheduleBox() {
  ...
  if (!box) {
    ...
    const field = courseTeacherSelect.closest(".course-modal__field");
    const note = panel.querySelector(".course-modal__note");

    if (note) {
      panel.insertBefore(box, note);
    } else if (field) {
...
function renderTeacherSchedule() {
  ...
  const selectedTranslations = getCourseTranslations();
  const schedule = teacherSchedules[courseTeacherSelect.value] || defaultSchedule;

  box.innerHTML = `
    <h4>${selectedTranslations["modal.scheduleTitle"] || "Время занятий"}</h4>
    <ul>${schedule.map((item) => `<li>${item}</li>`).join("")}</ul>
  `;
}

function removeTeachersButton() {
  document.querySelectorAll(".course-modal__secondary").forEach((button) => {
    button.remove();
  });
}
...
function setupCourseSliderControls() {
  ...
  const getVisibleCount = () => window.matchMedia("(max-width: 760px)").matches ? 1 : Math.min(4, cards.length);
...
if (courseTeacherSelect) {
  courseTeacherSelect.addEventListener("change", renderTeacherSchedule);
}
...
if (courseLanguageSwitcher) {
  courseLanguageSwitcher.addEventListener("change", (event) => {
